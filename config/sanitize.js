...
const sanitize = {
  ...
  verifyCezar: function(cezar) {
    if(!cezar || cezar === '') return true;
    const regex = /(https:\/\/msc\.com\.pl\/cezar)/;
    if(!regex.test(cezar)) return false;
    const regex2 = /\$\{\}/;
    if(regex2.test(cezar)) return false;
    return true;
  }
}
